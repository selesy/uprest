 ...
 */
@Slf4j
public class HttpRequestResolver extends AbstractParameterResolver implements ChainableParameterResolver {
	...
	/**
	 * Essentially provides a toString() method that returns a pretty-printed
	 * list of the names of the passed enum elements.
	 * 
	 * @param values
	...
	String getEnumNameList(Enum<?>[] values) {
		return Arrays.stream(values).map((x) -> x.name()).collect(Collectors.joining(", "));
	}
	...
	String getJoinedArrayOfHeadersAsString(Header[] headers) {
		return Arrays.stream(headers).map((h) -> h.getName() + ":" + h.getValue()).collect(Collectors.joining(", "));
	}
	...
	String getJoinedArrayOfStrings(String[] members) {
		return Arrays.stream(members).collect(Collectors.joining(", "));
	}
	...
	String[] permutePaths(ExtensionContext extensionContext) {
		...
		return Stream.of(basePaths).flatMap((p) -> Stream.of(typePaths).map((t) -> p + t))
				.flatMap((p) -> Stream.of(methodPaths).map((m) -> p + m)).collect(Collectors.toList())
